...
<head>
    ...
    <title>{{ $profile->name ?? 'Portfolio' }} | Digital Solutions</title>
...
<body class="bg-slate-50 text-slate-900 selection:bg-indigo-100 selection:text-indigo-600">
    <nav class="fixed w-full z-50 bg-white/80 backdrop-blur-md border-b border-slate-100">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16 items-center">
                ...
                <div>
                    <a href="{{ route('login') }}" class="bg-indigo-600 text-white px-5 py-2 rounded-full text-sm font-semibold hover:bg-indigo-700 transition-all soft-shadow">Admin Access</a>
                </div>
            </div>
        </div>
    </nav>
    ...
    <footer class="bg-white border-t border-slate-100 py-12">
        <div class="max-w-7xl mx-auto px-4 text-center">
            <p class="text-slate-400 text-sm">© {{ date('Y') }} {{ $profile->name ?? 'Willyan' }}. Built with ❤️ for Digital Excellence.</p>
        </div>
    </footer>
</body>
</html>
