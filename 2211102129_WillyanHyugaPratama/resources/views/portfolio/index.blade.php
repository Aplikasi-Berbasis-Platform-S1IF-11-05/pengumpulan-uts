        ...
        <div class="flex flex-col md:flex-row items-center gap-12">
            <div class="flex-1 space-y-6">
                ...
                <h1 class="text-5xl md:text-6xl font-bold leading-tight">
                    Hi, I'm <span class="gradient-text">{{ $profile->name ?? 'Willyan' }}</span>
                </h1>
                <p class="text-lg text-slate-600 leading-relaxed max-w-2xl">
                    {{ $profile->bio ?? 'Mahasiswa Teknik Informatika yang berfokus pada Desain, Machine Learning dan Solusi Teknologi Digital.' }}
                </p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="p-8 rounded-3xl border border-slate-100 hover:border-indigo-100 hover:bg-indigo-50/30 transition-all group">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="font-bold text-lg text-slate-800">{{ $skill->name }}</h3>
                    <span class="text-indigo-600 font-bold text-sm">{{ $skill->level }}%</span>
                </div>
                <div class="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div class="h-full bg-gradient-saas transition-all duration-1000" style="width: {{ $skill->level }}%"></div>
                </div>
                <p class="mt-4 text-sm text-slate-500">{{ $skill->category }} level expertise</p>
            </div>
            @endforeach
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
            ...
            <div class="group bg-white rounded-[2.5rem] p-8 soft-shadow border border-white hover:border-indigo-100 transition-all">
                <div class="flex justify-between items-start mb-6">
                    <div class="space-y-1">
                        <span class="text-xs font-bold uppercase tracking-widest text-indigo-500">{{ $project->role }}</span>
                        <h3 class="text-2xl font-bold text-slate-800">{{ $project->title }}</h3>
                    </div>
                    <span class="bg-slate-100 text-slate-600 px-3 py-1 rounded-full text-xs font-bold">{{ $project->year }}</span>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="bg-white/10 backdrop-blur-lg border border-white/10 rounded-3xl p-10 hover:bg-white/20 transition-all">
                <div class="flex gap-6">
                    ...
                    <div>
                        <h3 class="text-xl font-bold mb-2 text-indigo-100">{{ $achievement->title }}</h3>
                        <p class="text-indigo-200/80 leading-relaxed mb-4">{{ $achievement->description }}</p>
